  ...
  return (
    <div className="flex flex-col h-[100dvh] w-full max-w-md mx-auto bg-white dark:bg-slate-900 shadow-2xl sm:border-x sm:border-slate-200 dark:border-slate-800">
      ...
      <div className="p-4 sm:p-6 flex-1 overflow-y-auto pb-24 space-y-6">
        <section className="bg-slate-100 dark:bg-slate-800/80 p-6 rounded-3xl shadow-lg border border-slate-200 dark:border-slate-700/50 backdrop-blur-sm">
          <div className="flex justify-between items-end mb-4">
            <div>
              <p className="text-xs font-medium text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Gasto</p>
              <p className="text-2xl font-bold text-slate-900 dark:text-slate-100">R$ {spent.toFixed(2)}</p>
            </div>
            <div className="text-right">
              <p className="text-xs font-medium text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Restante</p>
              <p className={`text-lg font-bold ${remaining >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                R$ {remaining.toFixed(2)}
              </p>
            </div>
          </div>
          <div className="h-3 w-full bg-white dark:bg-slate-900 rounded-full overflow-hidden border border-slate-200 dark:border-slate-700/50">
            <div 
          ...
          <p className="text-center text-xs text-slate-400 dark:text-slate-500 mt-3 font-medium">Orçamento Total: R$ {block.totalAmount.toFixed(2)}</p>
        </section>

        <section>
          <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-4">Transações do Bloco</h2>
          {transactions.length === 0 ? (
            <div className="text-center p-8 bg-slate-100 dark:bg-slate-800/30 rounded-3xl border border-slate-200 dark:border-slate-700 border-dashed">
              <p className="text-slate-400 dark:text-slate-500 dark:text-slate-400 text-sm">Nenhuma transação registrada neste bloco.</p>
            </div>
          ) : (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400 mt-0.5">{cat?.name} • {format(new Date(t.date), 'dd/MM/yyyy')}</p>
                      </div>
                    </div>
